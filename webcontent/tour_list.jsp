...
if (search != null) {
	...
	sql = "select seq, thumbnail, place_name, simple_content, place_hit from tour";
	pstmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
	rs = pstmt.executeQuery();
}
...
%>
<div class="Tour_list">
	<form action="/sns1/main.jsp" id="search_form" method="get">
		<input type="hidden" name="target" value="tour_list" />
		<div class="search_box">
			<span class="search_icon"><i class="fa fa-search"></i></span> <input
				type="search" name="search" class="search" placeholder="Search...">
	...
	<ul class="Tour_list_box">
		...
		%>
		<%
		if (tourLimit > lastCheck) {
		%>
		<%
		end = start + lastCheck - 1;
		%>
		<%
		}
		%>
		...
		<li>
			<button type="button" class="like_btn"
				data-seq="<%=rs.getInt("seq")%>" title="좋아요">
				<%
				...
				%>
			</button> <a href="main.jsp?target=tour_page&tour_seq=<%=rs.getInt("seq")%>">
				<div class="img_box">
	...
	<div class="numbtn">
		<ul>
			...
			%>
			<li><a
				href="main.jsp?target=tour_list&pageNum=<%=currentPageSetup%>"><i
			...
			%>
			<li><a
				href="main.jsp?target=tour_list&pageNum=<%=i%>&search=<%=search%>"><%=i%></a>
			</li>
			<%
			}
			%>
			...
			<li>
				...
				%> <a href="main.jsp?target=tour_list&pageNum=<%=currentPageNext%>"><i
